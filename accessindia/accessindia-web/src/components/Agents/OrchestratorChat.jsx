    ...
    <section className="flex flex-col h-[calc(100vh-12rem)] md:h-[calc(100vh-10rem)] max-w-4xl mx-auto" aria-label="Chat interface">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 md:space-y-4 pb-4" role="log" aria-live="polite" aria-label="Chat messages">
        {messages.length === 0 && (
          <div className="flex items-center justify-center h-full px-4">
            <div className="text-center max-w-md">
              <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center mx-auto mb-4">
                <Sparkles className="w-7 md:w-8 h-7 md:h-8 text-orange-400" aria-hidden="true" />
              </div>
              <h2 className="text-lg md:text-xl font-bold text-slate-200 mb-2">Welcome to AccessIndia AI</h2>
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'} px-1`}>
              <div className={`max-w-[85%] md:max-w-2xl p-3 md:p-4 shadow-lg ${
      ...
      {isListening && transcript && (
        <div className="px-3 md:px-4 py-2 bg-orange-500/10 border border-orange-500/30 rounded-xl mb-2 text-sm text-orange-300">
          <span aria-live="assertive">🎤 {transcript}</span>
        </div>
      )}
      {sttError && (
        <div className="px-3 md:px-4 py-2 bg-rose-500/10 border border-rose-500/30 rounded-xl mb-2 text-sm text-rose-400" role="alert">
          ⚠️ {sttError}
        </div>
      )}

      {/* Input Area */}
      <div className="flex items-center gap-2 md:gap-3 bg-slate-800/80 border border-slate-700 rounded-2xl p-2 md:p-3">
        <button
        ...
        <button
          ...
          className="touch-target flex items-center justify-center p-3 rounded-xl bg-orange-500 hover:bg-orange-600 disabled:opacity-40 text-white transition-all shadow-lg shadow-orange-500/20"
